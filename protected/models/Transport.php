...
class Transport extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, state_number, fuel_consumption', 'required'),
			array('fuel_consumption', 'numerical'),
			array('name', 'length', 'max'=>50),
			array('state_number', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, state_number, fuel_consumption', 'safe', 'on'=>'search'),
		);
	}
